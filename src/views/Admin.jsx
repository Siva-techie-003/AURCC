...
const Admin = () => {
    ...
    return (
        <div className="bg-gradient-to-br from-indigo-100 via-blue-50 to-white min-h-screen">
            <div className="text-center md:py-6 py-3 bg-blue-700 bg-opacity-90">
...
const AdminStaffCard = ({ member }) => {
    ...
    return (
        <div className="admin-card bg-white/60 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden flex flex-col items-center border border-blue-200 transition-all duration-300 w-80 h-96">
            <div className="w-full flex justify-center pt-4 sm:pt-6">
                <div className="w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-blue-400 shadow-lg bg-white flex items-center justify-center">
                    {member.image && !error ? (
                        <img
                            src={`/${member.image}`}
                            alt={member.name}
                        ...
                        <span className="text-3xl sm:text-4xl lg:text-5xl text-blue-300">👤</span>
                    )}
                </div>
            </div>
            <div className="w-full bg-gradient-to-r from-blue-900 to-blue-500 py-3 sm:py-4 px-3 flex flex-col items-center justify-center flex-grow">
                ...
                <p className="text-xs lg:text-sm xl:text-base text-blue-100 font-sans text-center leading-tight">{member.position || 'Position not available'}</p>
            </div>
        </div>
    );
};
