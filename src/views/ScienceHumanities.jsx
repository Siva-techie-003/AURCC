    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-50 text-left">
            ...
            <section className="relative h-64 sm:h-80 lg:h-[50vh] overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-r from-blue-950/80 to-indigo-900/70"></div>
                <div className="container mx-auto h-full flex flex-col justify-center relative z-10 px-4">
                    ...
                    <p className="text-lg lg:text-xl text-blue-100 font-medium opacity-90 max-w-2xl">Bridging pure sciences with engineering innovation.</p>
                </div>
            </section>
            ...
            <main className="max-w-7xl mx-auto py-16 px-4 space-y-24">
                ...
                <section id="about-department" className="animate-fadeIn">
                    <header className="flex flex-col md:flex-row md:items-end justify-between border-b-4 border-indigo-900 pb-8 mb-12 gap-6">
                        <div className="max-w-4xl">
                            <h2 className="text-2xl lg:text-4xl font-black text-indigo-950 uppercase tracking-tight">Academic Overview</h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                        ...
                        <div className="space-y-8">
                            <div className="bg-indigo-900 text-white p-10 rounded-3xl shadow-xl flex flex-col gap-4">
                                <h3 className="text-2xl font-black uppercase tracking-tight text-blue-300">Our Vision</h3>
                                <p className="text-indigo-50 font-medium leading-relaxed">{department.vision}</p>
                            </div>
                            <div className="bg-white border-2 border-indigo-100 p-10 rounded-3xl">
                                <h3 className="text-xl font-bold text-indigo-950 uppercase mb-6 flex items-center gap-3">
                ...
                <section id="faculty" className="animate-fadeInUp">
                    <header className="flex flex-col md:flex-row md:items-end justify-between border-b-4 border-indigo-900 pb-8 mb-12 gap-6">
                        <div className="max-w-4xl">
                            <h2 className="text-2xl lg:text-4xl font-black text-indigo-950 uppercase tracking-tight">Faculty Team</h2>
                            <div className="h-1.5 w-20 bg-blue-600 mt-4 rounded-full"></div>
                        </div>
                    </header>
                    ...
                    <div className="space-y-16">
                        {/* HOD */}
                        <div>
                            <h3 className="text-xs font-black text-indigo-400 uppercase tracking-widest text-center mb-8">Head of Department</h3>
                            <div className="flex justify-center">
                                {(Array.isArray(department?.faculty?.hod_desk) ? department.faculty.hod_desk : []).map((staff, i) => (
                                    ...
                                    >
                                        ...
                                        <h4 className="text-xl font-black text-indigo-950 uppercase tracking-tight">{staff.name}</h4>
                                        <p className="text-xs font-black text-indigo-500 uppercase tracking-widest mt-1">{staff.position}</p>
                                        <span className="mt-6 px-6 py-2 bg-indigo-50 text-indigo-600 rounded-full font-bold text-xs">View Profile</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 className="text-xs font-black text-indigo-400 uppercase tracking-widest text-center mb-10">Academic Faculty</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                                {(Array.isArray(department?.faculty?.assistant_professors) ? department.faculty.assistant_professors : []).map((staff, i) => (
                                    ...
                                    >
                                        ...
                                        <h4 className="text-base font-black text-indigo-950 uppercase tracking-tight leading-tight">{staff.name}</h4>
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1 mb-4">{staff.position}</p>
                                        <div className="mt-auto pt-4 border-t border-gray-50 w-full text-[10px] font-black text-indigo-600 uppercase tracking-widest">Profile Details</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="overflow-y-auto p-8 sm:p-12">
                            <div className="flex flex-col md:flex-row gap-10">
                                <div className="md:w-1/3 flex flex-col items-center">
                                    ...
                                    <h4 className="text-2xl font-black text-indigo-950 text-center uppercase tracking-tighter leading-tight">{selectedStaff.name}</h4>
                                    <p className="text-indigo-600 font-bold uppercase tracking-widest text-xs mt-2">{selectedStaff.position}</p>
                                ...
                                <div className="flex-grow space-y-10 text-sans">
                                    {selectedStaff.education?.length > 0 && (
                                        <section>
                                            <h3 className="text-lg font-black text-indigo-950 mb-4 border-b-2 border-indigo-50 pb-2 flex items-center gap-2">
                                                <span className="text-sm">🎓</span> Education
                                            </h3>
                                            <div className="space-y-4">
                                                {selectedStaff.education.map((edu, i) => (
                                                    <div key={i} className="flex gap-4">
                                                        <div className="text-indigo-600 font-bold text-sm shrink-0">{edu.year}</div>
                                                        <div>
                                        ...
                                        <section>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {selectedStaff.research_interests.map((int, i) => (
                                                    <span key={i} className="px-3 py-1.5 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-full border border-indigo-100 uppercase tracking-widest">{int}</span>
                                                ))}
                                            </div>
                                        </section>
                                    )}
